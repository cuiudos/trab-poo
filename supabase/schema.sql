-- Colégio Jardim das Acácias — schema Supabase
...
-- Perfil vinculado ao auth.users (login seguro Supabase Auth)
...
-- Dados acadêmicos do aluno (nota/faltas)
...
-- Escolas: autenticados da escola podem ler
create policy "escolas_select" on public.escolas
  for select to authenticated
  using (id = public.minha_escola_id());
...
create policy "turmas_select_escola" on public.turmas
  for select to authenticated
  using (escola_id = public.minha_escola_id());
